#!/usr/bin/env bash
### In this script we will run the ABC method! This uses the hg19 merged celltype bams, a narrowPeak file (which we will generate here) and ABC'#!/bin/sh
### default HiC data (hg19). In the future we will add in using ChIP-seq data and our own hg38 HiC data.
...
# I ALSO UPDATED THE GENES LIST FILE USED TO DEFINE GENES AND THEIR COORDS (to the gencode_v32.hg19 file I made)
...
do
  ...
  python /nfs/lab/ABC/ABC-Enhancer-Gene-Prediction/src/makeCandidateRegions.py \
  --narrowPeak "${ABC_dir}/inputs/peak_calls_hg19/${ct}_peaks.narrowPeak" \
  --bam "${ABC_dir}/inputs/ATAC_bams/hg19_liftover/${ct}.bam" \
  --outDir $ct_outdir \
  --chrom_sizes /nfs/lab/ABC/ABC-Enhancer-Gene-Prediction/reference/chr_sizes \
  --regions_blocklist /nfs/lab/ABC/ABC-Enhancer-Gene-Prediction/reference/wgEncodeHg19ConsensusSignalArtifactRegions.bed \
  --regions_includelist /nfs/lab/ABC/references/gene_coords.gencodev32.hg19.TSS500bp.bed \
  ...
  python /nfs/lab/ABC/ABC-Enhancer-Gene-Prediction/src/run.neighborhoods.py \
  --candidate_enhancer_regions "${ct_outdir}/${ct}_peaks.narrowPeak.${ct}.bam.Counts.bed" \
  --genes /nfs/lab/ABC/references/gene_coords.gencodev32.hg19.bed \
  ...
  python /nfs/lab/ABC/ABC-Enhancer-Gene-Prediction/src/predict.py \
  --enhancers "${ct_outdir}/Neighborhoods/EnhancerList.txt" \
  --genes "${ct_outdir}/Neighborhoods/GeneList.txt" \
  --HiCdir /nfs/lab/ABC/references/BroadHiCdata/ \
  --chrom_sizes /nfs/lab/ABC/ABC-Enhancer-Gene-Prediction/reference/chr_sizes \
  --hic_resolution 5000 \
